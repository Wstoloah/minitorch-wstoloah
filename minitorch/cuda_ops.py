...
def device_jit(fn: Fn, **kwargs: Any) -> Fn:
    """Wraps a function with JIT compilation for CUDA backend."""
    return _jit(device=True, **kwargs)(fn)  # type: ignore


def jit(fn: Fn, **kwargs: Any) -> FakeCUDAKernel:
    """JIT compile a function for CUDA backend."""
...
class CudaOps(TensorOps):
    ...
    def map(fn: Callable[[float], float]) -> MapProto:
        ...
        def ret(a: Tensor, out: Optional[Tensor] = None) -> Tensor:
            ...
            ) // THREADS_PER_BLOCK  # ceiling division  == ceil(a/b)
...
def _sum_practice(out: Storage, a: Storage, size: int) -> None:
    ...
    pos = (
        cuda.threadIdx.x
    )  # position of the thred inside the block (range 0 to BLOCK_DIM-1)

    if pos == 0:
        cache[cuda.blockIdx.x] = 0.0
    cuda.syncthreads()

    if i < size:  # each thread checks if its index i is in bounds
        cuda.atomic.add(cache, cuda.blockIdx.x, a[i])

    cuda.syncthreads()
    if pos == 0 and cuda.blockIdx.x < out.size:  # all threads in block are done adding
...
def sum_practice(a: Tensor) -> TensorData:
    """Compute block-wise partial sums of a 1D tensor using a CUDA kernel.

    Each CUDA block sums a chunk of the input using shared memory.
    The result is returned in a GPU tensor of fixed size 2.
    """
    (size,) = a.shape
    threadsperblock = THREADS_PER_BLOCK
...
def tensor_reduce(
    ...
    def _reduce(
        ...
        out_shape: Shape,
        out_strides: Strides,
        out_size: int,  # total number of output values
        a_storage: Storage,
        a_shape: Shape,
        a_strides: Strides,
        reduce_dim: int,
        ...
        pos = cuda.threadIdx.x  # thread index within the block

        if pos == 0:
            cache[out_pos] = reduce_value  # starting value (e.g. 0.0 for sum)
        cuda.syncthreads()
...
def mm_practice(a: Tensor, b: Tensor) -> TensorData:
    """Perform a square matrix multiplication using a CUDA kernel.

    Computes the product of two size × size tensors and returns the result
    in a GPU tensor using fixed thread and block configuration.
    """
    (size, _) = a.shape
...
def _tensor_matrix_multiply(
    ...
    * Only write to global memory once per kernel.
    ...
    for phase in range((a_shape[2] + BLOCK_DIM - 1) / BLOCK_DIM):
        a_shared[pi, pj] = numba.float64(0.0)
        b_shared[pi, pj] = numba.float64(0.0)
